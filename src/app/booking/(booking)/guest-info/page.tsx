...
export default async function GuestInfoPage({
  ...
  const t = await getTranslations("booking.guestInfo");
  ...
  return (
    <StepShell step="guest-info" backHref={`/party?date=${date}&time=${slot.id}&plan=${plan.id}`}>
      <div className="space-y-1">
        <h1 className="font-heading text-2xl font-semibold text-foreground">{t("heading")}</h1>
        <p className="text-sm text-muted-foreground">
          {formatBusinessDate(date)} · {slot.startTimeLabel} · {plan.name} · {t("partyCount", { count: partySize })}
        </p>
      </div>
      <GuestInfoForm restaurant={restaurant} businessDate={date} slotId={slot.id} planId={plan.id} partySize={partySize} />
    </StepShell>
  );
}
